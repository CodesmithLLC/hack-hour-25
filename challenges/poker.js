...
function poker(hand1, hand2) {
  const playerOne = hand1.reduce((accum, element) => {
    accum[element] = (accum[element] || 0) + 1
    return accum;
  },{})
  const playerTwo = hand2.reduce((accum, element) => {
    accum[element] = (accum[element] || 0) + 1
    return accum;
  },{})
  ...
  Object.entries(playerOne).forEach((element) => {
    const [key, value] = element;
    if (value === 3) playerOneRank = 1
  })

...
function anagrams(string) {
  ...
  function innerFunc(anagram, str) {
    if (!str) output[anagram] = 1;

    // Loop through string
    for (let i = 0; i < str.length; i += 1) {
      innerFunc(anagram + str[i], str.slice(0, i) + str.slice(i + 1));
    }
  }

  // Invoke inner function passing in empty string and input string
  innerFunc('', string);

  // Return keys from output object
